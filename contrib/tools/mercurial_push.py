...
def push_review_hook(node):
    ...
    if 'REVIEWBOARD_URL' not in config:
        logging.error('You need to specify REVIEWBOARD_URL in the '
...
def get_ticket_url():
    ...
    if not ticket_url:
        repo_root = hghook.extcmd(['hg', 'root']).strip()
        logging.warning('%s/.hg/hgrc should specify', repo_root)
...
def push_review_hook_base(root, rbrepo, node, url, submitter):
    ...
    parent = node + '^1'
    logging.info('%d changesets received.', len(changesets))
    ...
    if is_approved(not_approved):
        ...
        if last_approved > -1:
            ...
            logging.info('hg push -r %s', changesets[last_approved])
...
def add_to_pending(revreq, root, ticket_url, ticket_prefixes,
    ...
    logging.warning('Cannot push until this review request'
                    ' is completed.')
    logging.warning('URL: %s', revreq.absolute_url)
...
def create(root, rbrepoid, submitter, url, changeset):
    ...
    logging.info('The review request must be completed before'
                 ' you can push again.')
    logging.info('URL: %s', revreq.absolute_url)
    return revreq
